...
base::Status MatmulLayer::check() const {
  ...
  if (!is_quant_layer_) {
    ...
    status = check_tensor_with_dim(get_weight(0), device_type_, base::DataType::DataTypeInt8,
                                   dim0_, dim1_);
    if (!status) {
      LOG(ERROR) << "The weight tensor error in the matmul layer.";
      return status;
...
base::Status MatmulLayer::set_bias(int32_t idx, int32_t& dim, const void* bias_ptr,
  ...
  if (!is_quant_layer_) {
    tensor::Tensor bias(base::DataType::DataTypeFp32, dim);
    ...
    // is quant layer
    tensor::Tensor bias(base::DataType::DataTypeInt8, dim);
    bias.set_device_type(device_type);
    CHECK(bias.assign(buffer));
    bias_.at(idx) = bias;
    ...
    scales_ = tensor::Tensor{base::DataType::DataTypeFp32, scale_nums, false, nullptr,
