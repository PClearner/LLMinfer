...
void Buffer::copy_from(const Buffer& buffer) const {
  ...
  CHECK(buffer_device != DeviceType::DeviceUnknown &&
        current_device != DeviceType::DeviceUnknown);

  if (buffer_device == DeviceType::DeviceCPU &&
      current_device == DeviceType::DeviceCPU) {
    return allocator_->memcpy(buffer.ptr(), this->ptr_, byte_size);
  } else if (buffer_device == DeviceType::DeviceCUDA &&
             current_device == DeviceType::DeviceCPU) {
    return allocator_->memcpy(buffer.ptr(), this->ptr_, byte_size,
                              MemcpyKind::MemcpyCUDA2CPU);
  } else if (buffer_device == DeviceType::DeviceCPU &&
             current_device == DeviceType::DeviceCUDA) {
    return allocator_->memcpy(buffer.ptr(), this->ptr_, byte_size,
                              MemcpyKind::MemcpyCPU2CUDA);
  } else {
    return allocator_->memcpy(buffer.ptr(), this->ptr_, byte_size,
                              MemcpyKind::MemcpyCUDA2CUDA);
  }
}
