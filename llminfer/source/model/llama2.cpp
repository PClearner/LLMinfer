...
base::Status LLama2Model::forward(const tensor::Tensor& input, const tensor::Tensor& pos_tensor,
  ...
  for (int32_t layer_idx = 0; layer_idx < config_->layer_num_; ++layer_idx) {
    // LOG(INFO) << "layer_idx:"<<layer_idx;
    attention_rms(layer_idx, input);
    // LOG(INFO) << "layer_idx:"<<layer_idx<<"||attention_rms is over";
    // attention (wq wk wv @ input)
    attention_qkv(layer_idx, pos_tensor);
    // LOG(INFO) << "layer_idx:"<<layer_idx<<"||attention_qkv is over";
    // multi-head attention
    attention_mha(layer_idx, pos_tensor);
    // LOG(INFO) << "layer_idx:"<<layer_idx<<"||attention_mha is over";
    // feed forward
    feed_forward(layer_idx, input);
    // LOG(INFO) << "layer_idx:"<<layer_idx<<"||feed_forward is over";
  }
  cls_logits(input);
...
void LLama2Model::init_mem() {
  ...
  // kv cache
  // LOG(INFO)<<"config_->seq_len_"<<config_->seq_len_;
